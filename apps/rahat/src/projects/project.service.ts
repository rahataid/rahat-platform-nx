  ...
  async handleProjectActions({ uuid, action, payload }) {
    switch (action) {
      ...
      case MS_ACTIONS.ELPROJECT.REQUEST_REDEMPTION:
        return this.client
          .send({ cmd: ProjectJobs.REQUEST_REDEMPTION, uuid }, payload)
          .pipe(timeout(500000));
      case MS_ACTIONS.ELPROJECT.UPDATE_REDEMPTION:
        return this.client
          .send({ cmd: ProjectJobs.UPDATE_REDEMPTION, uuid }, payload)
          .pipe(timeout(500000));
      case MS_ACTIONS.ELPROJECT.LIST_REDEMPTION:
        return this.client
          .send({ cmd: ProjectJobs.LIST_REDEMPTION, uuid }, payload)
          .pipe(timeout(500000));
      case MS_ACTIONS.ELPROJECT.GET_VENDOR_REDEMPTION:
        return this.client
          .send({ cmd: ProjectJobs.GET_VENDOR_REDEMPTION, uuid }, payload)
          .pipe(timeout(500000));
      case MS_ACTIONS.ELPROJECT.GET_VENDOR_REFERRER:
        return this.client
          .send({ cmd: BeneficiaryJobs.VENDOR_REFERRAL, uuid }, payload)
          .pipe(timeout(50000));
      default:
